...
void Level::initializeEntities(sf::RenderWindow* window, const rapidjson::Document& configDoc) {
	...
	*/
	...
	// The foreground
	// TODO: This is just copy pasted from background basically
	/*if (configDoc.HasMember("Foreground")) {
		const rapidjson::Value& foreground = configDoc["Foreground"];
		assert(foreground.IsObject());
		const rapidjson::Value& segments = foreground["segments"];
		assert(segments.IsInt());
		assert(foreground["filename"].IsString());
		for (int i = 0; i < segments.GetInt(); i++) {
			std::string FGFILEPATH = foreground["filename"].GetString();
			int position = foreground["filename"].GetStringLength() - 4;
			FGFILEPATH.insert(position, std::to_string(i + 1));
			rm->loadTexture(FGFILEPATH, IntRect(Vector2<int>(), Vector2<int>(Texture::getMaximumSize(), Texture::getMaximumSize())));
			sf::Sprite sprite(rm->getTexture(FGFILEPATH));
			sprite.setPosition((float)mMapBounds.width, 0.0f);
			mForegroundImages.push_back(sprite);
		}
	}
